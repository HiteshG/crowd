...
class IndiaEnglishHeuristicEngine:
    ...
    def react(
        ...
        reaction = Reaction(
            ...
            drop_beat=drop_beat,
            craving_mid=int(clamp(craving_mid, 1, 10)),
            craving_end=int(clamp(craving_end, 1, 10)),
    ...
    def _continue_reason(self, will_continue: bool, signals: dict[str, float], persona: Persona) -> str:
        positives = [
            ("the lead shows professional agency", signals["agency"] + signals["competence"]),
            ("career and status momentum are moving", signals["ambition"] + signals["status"]),
    ...
    def _pay_reason(
        ...
        if would_pay:
            if signals["ending_status_reveal"] >= signals["ending_cliffhanger"]:
                return "Pays because the gate blocks a status or professional reveal."
    ...
    def _prediction(self, signals: dict[str, float], persona: Persona, rng: random.Random) -> str:
        options = [
            ("The professional rival will be exposed in a public setting.", signals["public_vindication"] + signals["competence"]),
            ("The lead will choose career momentum over family pressure.", signals["family"] + signals["agency"] + persona.driver_intensity["identity"] * 0.3),
            ("A relationship confession will be interrupted by a workplace crisis.", signals["romance"] + signals["ending_romance_pressure"]),
            ("The hidden status or ownership truth will come out at the worst possible moment.", signals["ending_status_reveal"] + signals["status"]),
            ("The anonymous threat will connect the office plot to the family plot.", signals["cliffhanger"] + signals["family"]),
            ("The lead will win respect by solving the next practical problem herself.", signals["competence"] + signals["agency"]),
            ("A family secret will become the next emotional trap.", signals["family"] * persona.genre_affinity.get("family_drama", 0.0)),
            ("The next clue will reveal who is lying.", signals["cliffhanger"] * persona.genre_affinity.get("crime_mystery", 0.0)),
            ("The setback will turn into a revenge move.", signals["humiliation"] * persona.genre_affinity.get("revenge", 0.0)),
        ]
        return weighted_choice(rng, [(text, max(weight, 0.05)) for text, weight in options])
